...
# Parse dragon objects using python regex
dragon_blocks = re.findall(r'\{\s*id:\s*(\d+).*?scroll:\s*\"([^\"]+)\".*?\}', content, re.DOTALL)
print(f"Extracted {len(dragon_blocks)} dragon blocks")
