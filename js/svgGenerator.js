// 90s Retro Comic-Adventure Procedural Cartoon SVG Generator
// Angular asymmetric shapes, gangly proportions, deadpan expressions, limited retro palette (mustard, teal, rust, faded red)
...
export function renderDragonSVG(dragon, width = 300, height = 240) {
  ...
  // Procedural deterministic seeds based on dragon ID
  const seed1 = (id * 17) % 5;
  const seed2 = (id * 31) % 4;
  const seed3 = (id * 47) % 3;
  ...
  let headPolygon = "";
  if (seed1 === 0) headPolygon = "210,80 295,40 280,95 230,115 200,90"; // Long pointy snout
  else if (seed1 === 1) headPolygon = "215,70 285,55 295,95 220,110 205,85"; // Heavy blocky jaw
  else if (seed1 === 2) headPolygon = "205,85 275,35 290,75 240,110 195,95"; // Hook beak snout
  else if (seed1 === 3) headPolygon = "220,75 300,60 270,105 225,115 210,95"; // Triangular hammerhead
  else headPolygon = "210,80 280,45 285,90 235,110 205,90";

  // Eye Variations (Deadpan, Wide, Asymmetric Slits)
  let eyeMarkup = "";
  if (seed2 === 0) {
    // Deadpan heavy eyelids
    eyeMarkup = `
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="100%" height="100%" class="dragon-svg-illustration">
      <polygon points="20,20 ${width-20},10 ${width-10},${height-15} 15,${height-20}" fill="#191628" stroke="#e9c46a" stroke-width="3" />
      <polygon points="30,28 ${width-28},20 ${width-18},${height-25} 25,${height-28}" fill="none" stroke="#2a9d8f" stroke-width="2" stroke-dasharray="8,8" opacity="0.6" />
      ${paths}
    </svg>
  `;
}
